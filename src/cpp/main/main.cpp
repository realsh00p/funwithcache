  ...
  auto run(const int sundaram) -> Tests {
    ...
    for (auto &&s : suite) {
      ...
      case CACHE_MISS_STATIC_DEPTH_FIRST:
        ...
        break;
      case LOCALITY_HEAP:
        test->mutable_locality()->set_storage(Storage::HEAP);
        test->mutable_locality()->mutable_result()->CopyFrom(
            locality::run(Storage::HEAP));
        break;
      case LOCALITY_STATIC:
        test->mutable_locality()->set_storage(Storage::STATIC);
        test->mutable_locality()->mutable_result()->CopyFrom(
            locality::run(Storage::STATIC));
        break;
      }
    }
    return tests;
